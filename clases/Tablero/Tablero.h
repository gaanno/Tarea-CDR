...
void Tablero::disparar(int fila, int columna)
{
    ...
    if (celda == RANURA_VACIA) {
        ...
        cout << "Disparo fallido en fila " << (char)(fila + 'A') << ", columna " << (columna + 1) << "." << endl;
        ...
        cout << "Disparo exitoso en fila " << (char)(fila + 'A') << ", columna " << (columna + 1) << "." << endl;
    }
}


bool Tablero::esPosibleColocarBarco(const Barco& barco, int fila, int columna, char direccion)
{
